...
def getBeamData(cb):
    nSpans = simpledialog.askinteger(title="Input", prompt="Number of spans:", 
            parent=root, minvalue=1, initialvalue=3) 
    # nSpans = my_input('Number of spans', 3, int)
    cb.setNspans(nSpans)

    typicalSpan = simpledialog.askfloat(title='Input', prompt='Typical span', 
            parent=root, initialvalue=3.5)
    cb.setTypicalSpan(typicalSpan)
    beamLengths = []
    for i in range(nSpans):
        span = simpledialog.askfloat(title='Input', prompt='Length of beam ' + str(i+1), 
                parent=root, initialvalue=typicalSpan)
    ...
    messagebox.showinfo(message = msg_support)
    ...
    for jtIndex in range(nJoints):
        supportType = simpledialog.askinteger(title='Input', 
                ...
                parent=root, minvalue=Beam.FIXED, maxvalue=Beam.FREE, initialvalue=Beam.HINGE)
    ...
    for memberIndex in range(nSpans):
        ...
        while P != 0.0:
            x = simpledialog.askfloat(title='Input', 
                    ...
                    parent=root, minvalue=0.0) # TODO maxvalue as memberspan
            pL = PointLoad(P, x)
